(function () {
    ...
    }]).constant("emojiNames", (function (emojis) {
        // rEmojis RexGep Object need escaped chararcter(\\+1), but only display and matching item don't need slash slash (+1)
        var removedSlashEmojiNames = [];
        for (var i = 0, len = emojis.length; i < len; i++) {
            removedSlashEmojiNames.push(emojis[i].replace('\\', ''))
        }
        return removedSlashEmojiNames;
    })(emojis));
